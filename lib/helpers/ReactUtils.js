...
module.exports = function (j) {
  ...
  const hasModule = (path, _module) => path.findVariableDeclarators().filter(j.filters.VariableDeclarator.requiresModule(_module)).size() === 1 || path.find(j.ImportDeclaration, {
    type: 'ImportDeclaration',
    source: {
      type: 'StringLiteral'
    }
  }).filter(declarator => declarator.value.source.value === _module).size() === 1;

  const hasReact = path => hasModule(path, 'React') || hasModule(path, 'react') || hasModule(path, 'react/addons') || hasModule(path, 'react-native'); // ---------------------------------------------------------------------------
  // Finds all variable declarations that call React.createClass


  const findReactCreateClassCallExpression = path => j(path).find(j.CallExpression, {
    callee: REACT_CREATE_CLASS_MEMBER_EXPRESSION
  });

  const findReactCreateClass = path => path.findVariableDeclarators().filter(decl => findReactCreateClassCallExpression(decl).size() > 0);

  const findReactCreateClassExportDefault = path => path.find(j.ExportDeclaration, {
    default: true,
    declaration: {
      type: 'CallExpression',
      callee: REACT_CREATE_CLASS_MEMBER_EXPRESSION
  ...
  const findReactCreateClassModuleExports = path => path.find(j.AssignmentExpression, {
    left: {
      type: 'MemberExpression',
      object: {
        type: 'Identifier',
  ...
  const findReactComponentNameByParent = (path, parentClassName) => {
    const reactImportDeclaration = path.find(j.ImportDeclaration, {
      type: 'ImportDeclaration',
      source: {
        type: 'Literal'
      }
    }).filter(importDeclaration => hasReact(path));
    const componentImportSpecifier = reactImportDeclaration.find(j.ImportSpecifier, {
  ...
  const isMixinProperty = ({
    key,
    value
  }) => key.name === 'mixins' && value.type === 'ArrayExpression' && Array.isArray(value.elements) && value.elements.length;
  ...
  const createCreateReactClassCallExpression = properties => j.callExpression(j.memberExpression(j.identifier('React'), j.identifier('createClass'), false), [j.objectExpression(properties)]);

  const getComponentName = classPath => classPath.node.id && classPath.node.id.name; // ---------------------------------------------------------------------------
  // Direct methods! (see explanation below)


  const findAllReactCreateClassCalls = path => path.find(j.CallExpression, {
    callee: REACT_CREATE_CLASS_MEMBER_EXPRESSION
  }); // Mixin Stuff
  ...
  const containSameElements = (ls1, ls2) => {
    ...
    return ls1.reduce((res, x) => res && ls2.indexOf(x) !== -1, true) && ls2.reduce((res, x) => res && ls1.indexOf(x) !== -1, true);
  };

  const keyNameIsMixins = property => property.key.name === 'mixins';

  const isSpecificMixinsProperty = ({
    key,
    value
  }, mixinIdentifierNames) => key.name === 'mixins' && value.type === 'ArrayExpression' && Array.isArray(value.elements) && value.elements.every(elem => elem.type === 'Identifier') && containSameElements(value.elements.map(elem => elem.name), mixinIdentifierNames); // These following methods assume that the argument is
