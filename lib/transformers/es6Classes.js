...
function containsFlowProps(classBody) {
  return !!classBody.find(function (bodyElement) {
    return bodyElement.key.name.toLowerCase().includes('props');
  });
}
...
function transformEs6Classes(ast, j, options) {
  ...
  var modifications = reactClassPaths.forEach(function (p) {
    ...
    if (classBody) {
      ...
      (0, _annotateConstructor2.default)(j, classBody, propIdentifier);
